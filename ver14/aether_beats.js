...
// ==========================================
const PLAYLIST = [
    { id: 'track_01', title: '乾杯をしよう',     url: '../Musics/KanpaiSyo.mp3'   ,bgImage: '../Images/KanpaiSyo.jpg' },
...
function initTitleStars() {
    const preset = state.visualQuality === 'low'
        ? { starCount: 45 }
        : { starCount: 90 };
    titleStars = Array.from({ length: preset.starCount }, () => ({
...
async function beginTitleTransition() {
    ...
    const lines = [
        { t: '▸ AETHER BEATS SYSTEM v1.0.0',           cls: 'header' },
        { t: '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',  cls: 'header' },
    ...
    // 初回メニュー表示時は先頭トラックを自動選択する
    if (!state.selectedTrack && PLAYLIST.length > 0) {
        const randomIndex = Math.floor(Math.random() * PLAYLIST.length);
        const randomTrack = PLAYLIST[randomIndex];
        const trackButtons = document.querySelectorAll('#track-list button');
        const randomTrackButton = trackButtons[randomIndex] || null;
        await selectTrack(randomTrack, randomTrackButton);
    }
...
function getQualityPreset() {
    return QUALITY_PRESETS[state.visualQuality] || QUALITY_PRESETS.high;
}

function applyPreviewVolume() {
    if (!previewAudio) return;
    // 試聴音量をBGM設定に連動させる（プレビューなので少し控えめ）
    previewAudio.volume = Math.max(0, Math.min(1, state.bgmVolume * 0.85));
}
...
function triggerGoldConfetti() {
    const confettiCount = getQualityPreset().confettiCount;
...
function setupSettings() {
    ...
    const qualityToggleBtn = document.getElementById('quality-settings-toggle');
    const settingsMainView = document.getElementById('settings-main-view');
    const qualityWindow = document.getElementById('quality-settings-window');
    const qualityBackBtn = document.getElementById('quality-back-btn');
    const qualityCloseBtn = document.getElementById('quality-close-btn');

    function openQualitySettingsWindow() {
        settingsMainView?.classList.add('hidden');
        qualityWindow?.classList.remove('hidden');
    }

    function closeQualitySettingsWindow() {
        qualityWindow?.classList.add('hidden');
        settingsMainView?.classList.remove('hidden');
    }

    qualityToggleBtn?.addEventListener('click', openQualitySettingsWindow);
    qualityBackBtn?.addEventListener('click', closeQualitySettingsWindow);
    qualityCloseBtn?.addEventListener('click', closeSettings);

    document.getElementById('quality-only-high')?.addEventListener('click', () => setVisualQualityOnly('high'));
    document.getElementById('quality-only-low')?.addEventListener('click', () => setVisualQualityOnly('low'));

    document.getElementById('fps-60')?.addEventListener('click', () => setFpsLimit(60));
    document.getElementById('fps-45')?.addEventListener('click', () => setFpsLimit(45));
    document.getElementById('fps-30')?.addEventListener('click', () => setFpsLimit(30));
...
function updateQualitySettingsUI() {
    const setActive = (id, active) => document.getElementById(id)?.classList.toggle('active', active);

    setActive('quality-only-high', state.visualQuality === 'high');
    setActive('quality-only-low', state.visualQuality === 'low');

    setActive('fps-60', state.fpsLimit === 60);
    setActive('fps-45', state.fpsLimit === 45);
    setActive('fps-30', state.fpsLimit === 30);
}
...
async function openSettings() {
    ...
    document.getElementById('quality-settings-window')?.classList.add('hidden');
...
function spawnParticles(x, y, color, count) {
    const scaledCount = Math.max(0, Math.floor(count * getQualityPreset().particleScale));
...
function gameLoop() {
    ...
    // ── Background ──
    // 画面のクリア / 背景塗りつぶし
    if (state.currentBgImageObj) {
        // 1. 背景画像が設定されている場合、画像を画面いっぱいに描画
        ctx.drawImage(state.currentBgImageObj, 0, 0, canvas.width, canvas.height);
        
        // （オプション）ゲーム画面を見やすくするため、画像の上に半透明の黒を重ねる
        ctx.fillStyle = 'rgba(10, 10, 12, 0.6)'; // 透明度はお好みで調整
        ctx.fillRect(0, 0, canvas.width, canvas.height);
    } else {
        // 2. 背景画像が設定されていない場合、今と同じように単色で塗りつぶす
        ctx.fillStyle = config.colors.bg;
        ctx.fillRect(0, 0, canvas.width, canvas.height);
    }
